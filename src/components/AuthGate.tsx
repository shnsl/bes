  ...
  if (!user) {
    return (
      <main className="stage center">
        <ThemeToggle />
        <button type="button" className="google" aria-label="Google ile giriş" onClick={() => signIn(firebase)}>
          <GoogleMark />
        </button>
      </main>
    );
...
async function signIn(firebase: NonNullable<ReturnType<typeof getFirebase>>) {
